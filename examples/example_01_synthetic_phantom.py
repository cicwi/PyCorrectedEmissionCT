# -*- coding: utf-8 -*-
"""
Example tha reproduces the phantom used in:
...
def create_sino(
        ph, vol_att_in, vol_att_out, num_angles, psf, add_poisson=False,
        background_std=None):
    ...
    sino = num_photons * detector_angle_sr * corrct.create_sino(
            ph, angles, vol_att_in=vol_att_in, vol_att_out=vol_att_out, psf=psf)
...
(sino, angles) = create_sino(ph, vol_att_in, vol_att_out, 120, psf)
...
if apply_corrections:
    ...
    rec_sart = corrct.reconstruct('SART', sino, angles, vol_att_in=vol_att_in, vol_att_out=vol_att_out, psf=psf, lower_limit=0, data_term='l2', iterations=num_sart_iterations)
    ...
    rec_sirt = corrct.reconstruct('SIRT', sino, angles, vol_att_in=vol_att_in, vol_att_out=vol_att_out, psf=psf, lower_limit=0, data_term='l2', iterations=num_sirt_iterations)
    ...
    rec_cpkl = corrct.reconstruct('CP', sino / renorm_factor, angles, vol_att_in=vol_att_in, vol_att_out=vol_att_out, psf=psf, lower_limit=0, iterations=num_cp_iterations) * renorm_factor
    ...
    rec_cptvkl = corrct.reconstruct('CPTV', sino / renorm_factor, angles, vol_att_in=vol_att_in, vol_att_out=vol_att_out, psf=psf, lower_limit=0, lambda_reg=2e-1, iterations=num_cptv_iterations) * renorm_factor
